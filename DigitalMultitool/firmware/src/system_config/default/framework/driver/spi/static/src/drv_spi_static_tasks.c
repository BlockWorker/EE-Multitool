...
int32_t DRV_SPI1_ISRMasterEBM8BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    ...
    do {
        ...
        if (dObj->currentJob == NULL)
        {
            ...
            /* List the new job as processing*/
            currentJob->status = DRV_SPI_BUFFER_EVENT_PROCESSING;
            if (currentJob->dataLeftToTx +currentJob->dummyLeftToTx > PLIB_SPI_RX_8BIT_FIFO_SIZE(SPI_ID_4))
            {
                PLIB_SPI_FIFOInterruptModeSelect(SPI_ID_4, SPI_FIFO_INTERRUPT_WHEN_TRANSMIT_BUFFER_IS_1HALF_EMPTY_OR_MORE);
                PLIB_SPI_FIFOInterruptModeSelect(SPI_ID_4, SPI_FIFO_INTERRUPT_WHEN_RECEIVE_BUFFER_IS_1HALF_FULL_OR_MORE);
            }
            /* Flush out the Receive buffer */
        ...
        {   
            ...
            /* Interrupts should immediately become active again if they're in a fired condition */
            if ((SYS_INT_SourceStatusGet(INT_SOURCE_SPI_4_RECEIVE)) ||
                ...
                (SYS_INT_SourceStatusGet(INT_SOURCE_SPI_4_ERROR)))
            {
             ...
             /* If we're here then we know that the interrupt should not be firing again immediately, so re-enable them and exit*/
                SYS_INT_SourceEnable(INT_SOURCE_SPI_4_RECEIVE);
                SYS_INT_SourceEnable(INT_SOURCE_SPI_4_TRANSMIT);
            return 0;
        }
...
int32_t DRV_SPI2_ISRMasterEBM8BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    volatile bool continueLoop;
    /* Disable the interrupts */
    ...
    do {
        ...
        /* Execute the sub tasks */
            ...
            (currentJob->dataLeftToTx +currentJob->dummyLeftToTx != 0)
        {
            DRV_SPI2_MasterEBMSend8BitISR(dObj);
        }
        ...
        if (bytesLeft != 0)
        {
            DRV_SPI2_MasterEBMReceive8BitISR(dObj);
            bytesLeft = currentJob->dataLeftToRx + currentJob->dummyLeftToRx;
        }
        if (bytesLeft == 0)
        {
